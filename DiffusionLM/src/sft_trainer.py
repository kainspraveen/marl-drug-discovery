...
class FSDPSFTTrainer(object):
    ...
    def __init__(
        ...
        self._resumed_global_step = 0  # Track the step we resumed from
        ...
        # Determine training mode for checkpoint compatibility checks
        _is_lora = self.config.model.get("lora_rank", 0) > 0
        _mode_str = "LoRA" if _is_lora else "full-model"
        if self.device_mesh.get_rank() == 0:
            print(f"Training mode: {_mode_str}")

        # Auto-detect latest checkpoint from wandb if no explicit resume path
        if not self.resume_checkpoint_path:
            wandb_ckpt = self._find_latest_wandb_checkpoint()
            if wandb_ckpt is not None:
                self.resume_checkpoint_path = wandb_ckpt
                self.resume_training = True
                if self.device_mesh.get_rank() == 0:
                    print(f"Auto-detected compatible wandb checkpoint ({_mode_str}): {wandb_ckpt}")

        # If still no checkpoint, try local/HDFS
        if not self.resume_checkpoint_path:
            local_ckpt = self._find_latest_checkpoint()
            if local_ckpt is not None:
                self.resume_checkpoint_path = local_ckpt
                self.resume_training = True
                if self.device_mesh.get_rank() == 0:
                    print(f"Auto-detected compatible local checkpoint ({_mode_str}): {local_ckpt}")

        if not self.resume_checkpoint_path and self.device_mesh.get_rank() == 0:
            print(f"No compatible {_mode_str} checkpoint found — starting from scratch.")

        # build tokenizer first
        if self.resume_training and self.resume_checkpoint_path:
            # If resuming from checkpoint, use that path for tokenizer
        ...
        if self.resume_training and self.resume_checkpoint_path:
            self._build_model_optimizer(checkpoint_path=self.resume_checkpoint_path)
        else:
    ...
    def _build_model_optimizer(self, checkpoint_path=None):
        ...
        if checkpoint_path and not is_lora:
        ...
        with init_context():
            ...
            if is_lora:
                ...
                if checkpoint_path:
                    # Resume: load saved LoRA adapter from checkpoint
                    from peft import PeftModel
                    if self.device_mesh.get_rank() == 0:
                        print(f"Loading LoRA adapter from checkpoint: {checkpoint_path}")
                    self.model = PeftModel.from_pretrained(
                        self.model, checkpoint_path, is_trainable=True
                    )
                else:
                    # Fresh: create new LoRA adapter with random init
                    lora_config = {
    ...
    def _load_from_checkpoint(self, checkpoint_path):
        ...
        if os.path.exists(optimizer_state_path):
            ...
            with FSDP.state_dict_type(
                ...
                if self.device_mesh.get_rank() == 0:
                    optim_state = torch.load(optimizer_state_path, weights_only=False)
                else:
                    optim_state = None
        ...
        if self.device_mesh.get_rank() == 0:
            ...
            if wandb.run is not None:
                ...
                artifact = wandb.Artifact(
                    name=f"model-checkpoint-{self.config.trainer.experiment_name}", 
                    type="model",
                    metadata={
                        "step": step,
                        "epoch": self.current_epoch,
                ...
                artifact.add_dir(path)  # Adds the entire checkpoint folder
                wandb.log_artifact(artifact)
                mode_str = "LoRA" if is_lora else "full-model"
                print(f"Logged wandb artifact ({mode_str}) at step {step}")
        torch.distributed.barrier()
        

    def _find_latest_checkpoint(self):
        """Find the latest *compatible* checkpoint in local / HDFS directories.

        A checkpoint is compatible when its training mode (LoRA vs full-model)
        matches the current config.  Detection is based on the presence of
        ``adapter_config.json`` (LoRA) vs a regular model dir (full-model).
        """
        latest_checkpoint = None
        latest_step = -1
        current_is_lora = self.config.model.get("lora_rank", 0) > 0

        def _is_compatible_checkpoint(ckpt_path):
            """Return True if the checkpoint's training mode matches config."""
            has_adapter = os.path.isfile(os.path.join(ckpt_path, "adapter_config.json"))
            if current_is_lora and not has_adapter:
                if self.device_mesh.get_rank() == 0:
                    print(f"Skipping full-model checkpoint (need LoRA): {ckpt_path}")
                return False
            if not current_is_lora and has_adapter:
                if self.device_mesh.get_rank() == 0:
                    print(f"Skipping LoRA checkpoint (need full-model): {ckpt_path}")
                return False
            return True
        ...
        if os.path.exists(local_dir):
            ...
            for ckpt in checkpoints:
                ckpt_path = os.path.join(local_dir, ckpt)
                step = extract_step(ckpt)
                if step is not None and step > latest_step and _is_compatible_checkpoint(ckpt_path):
                    latest_step = step
        ...
        if latest_checkpoint is None:
            wandb_ckpt = self._find_latest_wandb_checkpoint()
            if wandb_ckpt is not None:
                latest_checkpoint = wandb_ckpt

        return latest_checkpoint

    def _find_latest_wandb_checkpoint(self):
        """Find and download the latest checkpoint from wandb.

        Checks whether the artifact's training mode (LoRA vs full-model)
        matches the current config.  If mismatched the artifact is skipped
        so training starts from scratch.
        """
        try:
            import wandb
            api = wandb.Api()
            ...
            artifact_name = f"model-checkpoint-{experiment_name}:latest"
            current_is_lora = self.config.model.get("lora_rank", 0) > 0

            entity = getattr(self.config.trainer, 'entity', None)
            if entity:
                full_artifact_name = f"{entity}/{project_name}/{artifact_name}"
            else:
                try:
            ...
            if self.device_mesh.get_rank() == 0:
                print(f"Looking for wandb artifact: {full_artifact_name}")

            artifact = api.artifact(full_artifact_name, type="model")

            artifact_is_lora = artifact.metadata.get("is_lora", None)
            if artifact_is_lora is not None and artifact_is_lora != current_is_lora:
                mode_artifact = "LoRA" if artifact_is_lora else "full-model"
                mode_current  = "LoRA" if current_is_lora  else "full-model"
                if self.device_mesh.get_rank() == 0:
                    print(
                        f"Wandb checkpoint is {mode_artifact}, but current config "
                        f"requests {mode_current} training. "
                        f"Skipping — will start from scratch."
                    )
                return None

            # Download to a local directory
            download_dir = os.path.join(
                self.config.trainer.default_local_dir, "wandb_checkpoint"
            )
            artifact_dir = artifact.download(root=download_dir)

            step = artifact.metadata.get("step", "unknown")
            epoch = artifact.metadata.get("epoch", "unknown")
            mode = (
                "LoRA" if artifact_is_lora
                else ("full-model" if artifact_is_lora is not None else "unknown-mode")
            )
            if self.device_mesh.get_rank() == 0:
                print(
                    f"Downloaded wandb checkpoint ({mode}): step={step}, "
                    f"epoch={epoch}, dir={artifact_dir}"
                )

            return artifact_dir
        except wandb.errors.CommError:
            if self.device_mesh.get_rank() == 0:
                print("No wandb checkpoint artifact found for this project.")
            return None
        except Exception as e:
            if self.device_mesh.get_rank() == 0:
                print(f"Could not fetch wandb checkpoint: {e}")
            return None
    ...
    def fit(self):
        ...
        if self.resume_training and self.resume_checkpoint_path:
            ...
            self._resumed_global_step = global_step
            if rank == 0:
                print(
        ...
        # Compute total training steps
        need_skip = self.resume_training and self._resumed_global_step > 0
        ...
        for epoch in range(start_epoch, self.config.trainer.total_epochs):
            ...
            steps_to_skip = 0
            if need_skip:
                steps_to_skip = self._resumed_global_step % self.steps_per_epoch
                if steps_to_skip > 0:
                    if rank == 0:
                        print(
                            f"Skipping {steps_to_skip} steps to resume at the right position in epoch {epoch}"
                        )
            ...
            remaining_steps = self.steps_per_epoch - steps_to_skip
